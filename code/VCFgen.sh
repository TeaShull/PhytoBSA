...
# picard BuildBamIndex INPUT=./output/$1_mu.sort.md.rg.bam O=./output/${1}_mu.sort.md.rg.bai &
# picard BuildBamIndex INPUT=./output/$1_wt.sort.md.rg.bam O=./output/${1}_wt.sort.md.rg.bai
# wait

# #GATK HC Variant calling
# gatk HaplotypeCaller -R $fa -I ./output/$1_mu.sort.md.rg.bam -I ./output/$1_wt.sort.md.rg.bam -O ./output/$1.hc.vcf -output-mode EMIT_ALL_CONFIDENT_SITES --native-pair-hmm-threads 20
...
if [ "$mutation" = 'recessive' ]; then 
	...
	grep -F -e $'0/1:0/0' -e '1/1:0/0' ./output/$1.ems.table.tmp > ./output/$1.ems.table
fi
...
#remove known snps
awk 'FNR==NR{a[$1$2];next};!($1$2 in a) || $1~/#CHROM/' $knownsnps ./output/$1.ems.table > ./output/$line.noknownsnps.table

#add headers
sed -i '1s/^/'chr'\t'pos'\t'ref'\t'alt'\t'gene'\t'snpEffect'\t'snpVariant'\t'snpImpact'\t'mu:wt_GTpred'\t'wt_alt'\t'wt_ref'\t'mu_alt'\t'mu_ref'\n/' ./output/$line.noknownsnps.table
